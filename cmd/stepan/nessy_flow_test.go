...
func TestMain(main *testing.M) {
	if os.Getenv("STEPAN_MAIN_NESSY_FAKE") == "1" {
		os.Exit(runCompositionNessyFake())
	}
	os.Exit(main.Run())
}
...
func runCompositionNessyFake() int {
	generation := os.Getenv("STEPAN_MAIN_NESSY_GENERATION")
	...
	observation := compositionACPObservation{
		...
		AuthConfigured:    os.Getenv("NESSY_CLI_DP_AUTH_TOKEN") == "test-auth-token",
	...
	for {
		...
		if mode := os.Getenv("STEPAN_MAIN_NESSY_LEAK"); mode != "" {
			token := os.Getenv("NESSY_CLI_DP_AUTH_TOKEN")
			if mode == "error" {
				_ = encoder.Encode(map[string]any{"jsonrpc": "2.0", "id": request.ID, "error": map[string]any{"code": -32603, "message": token}})
				return 0
			}
			candidate = `{"feature_id":"` + token + `"}`
		}
...
func compositionResponse(role string, turn int, artifactRoot string) (string, error) {
	...
	switch role {
	case "feature-id":
		return `{"feature_id":"nessy-provider-parity"}`, nil
	case "intent-author":
		if turn == 1 {
...
func writeCompositionObservation(observation compositionACPObservation) bool {
	directory := os.Getenv("STEPAN_MAIN_NESSY_OBSERVATIONS")
	if directory == "" {
		return false
	}
...
func TestNessyCompositionFullFlowGitPolicyAndDurableResume(t *testing.T) {
	...
	t.Setenv("STEPAN_MAIN_NESSY_GENERATION", "before-resume")
	...
	progress, err := application.StartFeature("Prove Nessy provider parity")
	if err != nil {
		t.Fatal(err)
	}
	...
	if blocked.Event != specflow.ControllerApprovalBlocked || len(blocked.Blocking) != 1 || blocked.Blocking[0].Path != "README.md" {
		t.Fatalf("Nessy approval blockers = %#v", blocked)
	}
	...
	t.Setenv("STEPAN_MAIN_NESSY_GENERATION", "after-resume")
	resumedApplication, resumedRegistry, resumedSession := newNessyCompositionApplication(t, root, config)
	t.Cleanup(func() {
	...
	for _, observation := range after {
		if observation.Generation == "after-resume" && observation.Role == "plan-author" {
			...
			if _, reused := before[observation.SessionID]; reused {
				t.Fatalf("resume reused Nessy session %q", observation.SessionID)
			}
	...
	if !freshPlanAuthor {
		t.Fatalf("resume did not start a fresh Nessy plan-author process: %#v", after)
	}
	...
	for _, review := range reviews {
		...
		if readErr != nil || !strings.Contains(string(data), `provider: "nessy"`) || !strings.Contains(string(data), `model: "default"`) {
...
func TestNessyCompositionCleanCreateGateDoesNotRollbackDirtyFiles(t *testing.T) {
	root := initializeCompositionRepository(t)
	observations := t.TempDir()
	t.Setenv("STEPAN_MAIN_NESSY_FAKE", "1")
	t.Setenv("STEPAN_MAIN_NESSY_OBSERVATIONS", observations)
	t.Setenv("STEPAN_MAIN_NESSY_GENERATION", "dirty-create")
	outside := filepath.Join(root, "README.md")
	...
	if !errors.Is(err, specflow.ErrRepositoryBlocked) {
		t.Fatalf("dirty Nessy create = %#v, %v", progress, err)
	}
...
func newNessyCompositionApplication(t *testing.T, root string, config agentConfig) (*specflow.ApplicationController, *specflow.SessionRegistry, *specflow.Session) {
	t.Helper()
...
func readCompositionObservations(t *testing.T, directory string) map[string]compositionACPObservation {
	...
	for _, entry := range entries {
		...
		if json.Unmarshal(data, &observation) != nil || observation.SessionID == "" {
			t.Fatalf("invalid Nessy observation %s: %s", entry.Name(), data)
		}
		if !observation.AuthConfigured {
			t.Fatal("thread did not receive configured auth")
		}
		if strings.Contains(strings.Join(observation.Prompts, "\n"), "test-auth-token") {
			t.Fatal("credential reached prompt")
		}
