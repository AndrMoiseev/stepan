...
их строгая локальная валидация при этом не изменяются. Перед этой валидацией
adapter удаляет из результата только известные свойства других веток transport
envelope; неизвестные свойства сохраняются и отклоняются узкой schema.
