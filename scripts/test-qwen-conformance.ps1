...
if ([string]::IsNullOrEmpty($AgentCliName) -or $AgentCliName.Trim() -cne $AgentCliName -or
    $AgentCliName -in @(".", "..") -or $AgentCliName.Contains("/") -or
    $AgentCliName.Contains("\") -or [IO.Path]::IsPathRooted($AgentCliName)) {
    Write-FailureResult "invalid_executable_name"
    exit 2
}
...
$runRoot = Join-Path ([IO.Path]::GetTempPath()) ("stepan-qwen-conformance-" + [guid]::NewGuid().ToString("N"))
$resultPath = Join-Path $runRoot "result.json"
$testLog = Join-Path $runRoot "go-test.log"
$crossLog = Join-Path $runRoot "darwin-cross-compile.log"
$crossBinary = Join-Path $runRoot "qwenapp-darwin-arm64.test"
$savedEnvironment = @{}
$names = @("STEPAN_QWEN_REAL_CLI", "STEPAN_QWEN_AGENT_CLI_NAME", "STEPAN_QWEN_RESULT", "GOOS", "GOARCH", "CGO_ENABLED")
...
try {
    [Environment]::SetEnvironmentVariable("STEPAN_QWEN_REAL_CLI", "1", "Process")
    [Environment]::SetEnvironmentVariable("STEPAN_QWEN_AGENT_CLI_NAME", $AgentCliName, "Process")
    [Environment]::SetEnvironmentVariable("STEPAN_QWEN_RESULT", $resultPath, "Process")

    $ErrorActionPreference = "Continue"
    & $goCommand.Source test -tags qwen_real_cli -run '^TestQwenRealCLIConformance$' `
        ./internal/agentruntime/qwenapp -count=1 -timeout ("{0}s" -f $TimeoutSeconds) *> $testLog
    $testExitCode = $LASTEXITCODE
    $ErrorActionPreference = "Stop"

    [Environment]::SetEnvironmentVariable("STEPAN_QWEN_REAL_CLI", $null, "Process")
    [Environment]::SetEnvironmentVariable("STEPAN_QWEN_AGENT_CLI_NAME", $null, "Process")
    [Environment]::SetEnvironmentVariable("STEPAN_QWEN_RESULT", $null, "Process")
    ...
    $ErrorActionPreference = "Continue"
    & $goCommand.Source test -c -tags qwen_real_cli -o $crossBinary `
        ./internal/agentruntime/qwenapp *> $crossLog
