...
func TestImplementationLoopDocumentationConfigurationExamplesValidate(t *testing.T) {
	...
	user := documentationImplementation(t, string(document), "implementation-config-user-example")
	project := documentationImplementation(t, string(document), "implementation-config-project-example")
	configuration, err := Merge(Sources{User: user, Project: project})
	if err != nil {
...
func documentationImplementation(t *testing.T, document, name string) json.RawMessage {
	t.Helper()
	start := "<!-- " + name + ":start -->"
	end := "<!-- " + name + ":end -->"
	...
	if !ok {
		t.Fatalf("%s JSON example has no implementation member", name)
	}
