...
func newRealCLIApplication(t *testing.T, workspace string, runtime *Runtime) realCLIApplication {
	...
	application, err := specflow.NewApplicationController(workspace, session, manager, flow, specflow.RuntimeIdentity{Provider: "qwen", Model: "cli-selected"})
	if err != nil {
...
func realCLIRuntimeIdentities(runtime *Runtime) []string {
	...
	for _, state := range runtime.threads {
		thread, ok := state.backend.(*qwenThread)
		if !ok {
			continue
		}
