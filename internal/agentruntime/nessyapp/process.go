...
// excludedToolsCSV is deliberately explicit as defense in depth: current Nessy
...
// Process owns one contained Nessy ACP child and the one transport artifact
...
func defaultDependencies() processDependencies {
	return processDependencies{
		...
		makeRoot: func() (string, error) {
			return os.MkdirTemp("", "stepan-nessy-")
		},
		remove:  os.RemoveAll,
		environ: os.Environ,
	}
...
func (process *Process) Start() error {
	...
	command.Env = isolatedEnv(baseEnvironment, process.config.AuthToken)
...
func (process *Process) preflight() (executable, workspace, artifact string, owned bool, err error) {
	...
	artifact, err = canonicalDirectory(artifact, "Nessy read-only transport root")
	if err != nil {
		return cleanup(err)
	}
