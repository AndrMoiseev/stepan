...
func ResolveExecutable() (string, error) {
	...
	if err != nil {
		failure := fmt.Errorf("resolve Nessy executable %q: %w", name, err)
...
func canonicalGitRoot(path string) (string, error) {
	root, err := canonicalDirectory(path, "Nessy workspace")
	if err != nil {
		return "", err
	}
	...
	if err != nil {
		return "", fmt.Errorf("verify Nessy Git root: %w", err)
	}
	rootInfo, err := os.Stat(root)
	if err != nil {
		return "", fmt.Errorf("stat Nessy Git root: %w", err)
	}
