...
func TestMain(main *testing.M) {
	if scenario := os.Getenv("GO_WANT_NESSYAPP_FAKE"); scenario != "" {
		os.Exit(runNessyFake(scenario))
	}
	os.Exit(main.Run())
}

func runNessyFake(scenario string) int {
	switch scenario {
	case "contract":
		...
		if err != nil || os.WriteFile(os.Getenv("STEPAN_NESSYAPP_METADATA"), data, 0o600) != nil {
			return 12
		}
	...
	case "startup-diagnostic":
		fmt.Fprintln(os.Stderr, "Authentication required while loading", os.Getenv("STEPAN_NESSYAPP_DIAGNOSTIC_SECRET"))
		return 23
	case "assignment-failure":
		...
		if err := os.WriteFile(os.Getenv("STEPAN_NESSYAPP_READY"), []byte("ready"), 0o600); err != nil {
			return 14
		}
		time.Sleep(30 * time.Second)
		return 0
	case "tree":
		return runNessyTreeFake()
	case "acp":
		return runNessyACPFake()
	default:
		return 10
	}
}
...
func diagnosticSecretValues() []string {
	return slices.DeleteFunc([]string{
		os.Getenv("NESSY_CLI_DP_AUTH_TOKEN"),
...
func TestLauncherPassesExactIsolatedContract(t *testing.T) {
	...
	t.Setenv("STEPAN_NESSYAPP_METADATA", metadataPath)
	t.Setenv("QWEN_API_KEY", "test-auth-value")
	t.Setenv("QWEN_CODE_ENABLE_AGENT_TEAM", "1")

	process := NewProcess(Config{AuthToken: testAuthToken(t), Workspace: workspace, JSONContract: testJSONContract}, artifact)
...
func TestReadOnlyProcessGetsEmptyOwnedRootAndRemovesIt(t *testing.T) {
	...
	t.Setenv("GO_WANT_NESSYAPP_FAKE", "contract")
	t.Setenv("STEPAN_NESSYAPP_METADATA", metadataPath)
	process := NewProcess(Config{AuthToken: testAuthToken(t), Workspace: makeGitRoot(t), JSONContract: testJSONContract}, "")
	if err := process.Start(); err != nil {
		t.Fatal(err)
	}
...
func TestSeparateProcessesReceiveNoSiblingArtifactRoot(t *testing.T) {
	...
	t.Setenv("GO_WANT_NESSYAPP_FAKE", "contract")
	t.Setenv("STEPAN_NESSYAPP_METADATA", firstMetadata)
	first := NewProcess(Config{AuthToken: testAuthToken(t), Workspace: workspace, JSONContract: testJSONContract}, firstRoot)
...
func TestExecutableResolutionUsesOnlyNessyPATH(t *testing.T) {
	testAuthToken(t)
	resolved, err := ResolveExecutable()
	if err != nil || !strings.HasPrefix(filepath.Base(resolved), "nessy") {
		t.Fatalf("resolve = %q, %v", resolved, err)
	}
	t.Setenv("PATH", t.TempDir())
	if _, err := ResolveExecutable(); err == nil {
		t.Fatal("missing nessy accepted")
	}
}

func TestInvalidRootsAndConfigAreRejectedBeforeLaunch(t *testing.T) {
	token := testAuthToken(t)
	workspace := makeGitRoot(t)
	...
	cases := []struct {
		...
		{name: "relative workspace", config: Config{AuthToken: token, Workspace: ".", JSONContract: testJSONContract}, artifact: t.TempDir()},
		{name: "not git root", config: Config{AuthToken: token, Workspace: t.TempDir(), JSONContract: testJSONContract}, artifact: t.TempDir()},
		{name: "fake git metadata", config: Config{AuthToken: token, Workspace: fakeGitRoot, JSONContract: testJSONContract}, artifact: t.TempDir()},
		{name: "missing contract", config: Config{AuthToken: token, Workspace: workspace}, artifact: t.TempDir()},
		{name: "missing artifact", config: Config{AuthToken: token, Workspace: workspace, JSONContract: testJSONContract}, artifact: filepath.Join(t.TempDir(), "missing")},
		{name: "artifact in workspace", config: Config{AuthToken: token, Workspace: workspace, JSONContract: testJSONContract}, artifact: filepath.Join(workspace, "artifact")},
		{name: "artifact contains workspace", config: Config{AuthToken: token, Workspace: workspace, JSONContract: testJSONContract}, artifact: parentArtifact},
	}
...
func TestCanonicalRootsRejectLinkIntoWorkspace(t *testing.T) {
	...
	process := NewProcess(Config{AuthToken: testAuthToken(t), Workspace: workspace, JSONContract: testJSONContract}, link)
...
func TestStartupFailuresAreClassifiedAndCleanOwnedRoot(t *testing.T) {
	t.Setenv("GO_WANT_NESSYAPP_FAKE", "assignment-failure")
	setDiagnosticSecrets(t)
	readyPath := filepath.Join(t.TempDir(), "ready")
	t.Setenv("STEPAN_NESSYAPP_READY", readyPath)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			process := newProcess(Config{AuthToken: testAuthToken(t), Workspace: makeGitRoot(t), JSONContract: testJSONContract}, "", deps)
			err := process.Start()
...
func TestWaitAndCloseAreIdempotent(t *testing.T) {
	t.Setenv("GO_WANT_NESSYAPP_FAKE", "wait")
	setDiagnosticSecrets(t)
	...
	for err := range errorsSeen {
		...
		for _, secret := range append(diagnosticSecretValues(), process.config.AuthToken) {
			if strings.Contains(err.Error(), secret) {
...
func runNessyTreeFake() int {
	switch os.Getenv("STEPAN_NESSYAPP_TREE_LEVEL") {
	case "":
		command := exec.Command(os.Args[0])
		command.Env = append(os.Environ(), "STEPAN_NESSYAPP_TREE_LEVEL=child")
	...
	case "child":
		command := exec.Command(os.Args[0])
		command.Env = append(os.Environ(), "STEPAN_NESSYAPP_TREE_LEVEL=grandchild")
		if err := command.Start(); err != nil {
			return 52
		}
		data, err := json.Marshal([]int{os.Getppid(), os.Getpid(), command.Process.Pid})
		if err != nil || os.WriteFile(os.Getenv("STEPAN_NESSYAPP_PID_FILE"), data, 0o600) != nil {
			return 53
		}
		time.Sleep(30 * time.Second)
	case "grandchild":
...
func waitForNessyPIDs(t *testing.T, path string) []int {
	t.Helper()
	...
	t.Fatal("fake Nessy process tree did not report PIDs")
	return nil
}
