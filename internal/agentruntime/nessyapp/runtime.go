...
// Runtime owns a set of independent contained Nessy processes. Processes are
...
// StartRuntime creates a lazy Nessy runtime. Executable resolution, process
...
func newRuntime(config Config, deps runtimeDependencies) *Runtime {
	...
	if deps.startThread == nil {
		deps.startThread = startNessyThread
	}
	if deps.after == nil {
...
func (runtime *Runtime) lookup(value agentruntime.Thread) (*threadState, error) {
	...
	if !ok || handle == nil || handle.identity != runtime.identity || handle.state == nil || handle.generation == 0 {
		return nil, errors.New("invalid Nessy thread handle")
	}
	runtime.mu.Lock()
...
func (runtime *Runtime) validateStateLocked(state *threadState) error {
	...
	if state == nil || state.closed || runtime.threads[state.generation] != state {
		return errors.New("invalid Nessy thread handle")
	}
	return nil
}
...
func safeRuntimeError(action string, err error) error {
	...
	if len(contexts) > 0 {
		return fmt.Errorf("nessy %s (%s): %w", action, strings.Join(contexts, "; "), errors.Join(categories...))
	}
	return fmt.Errorf("nessy %s: %w", action, errors.Join(categories...))
}
