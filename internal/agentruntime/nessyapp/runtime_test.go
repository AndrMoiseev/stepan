...
func testRuntime(t *testing.T, factory *fakeRuntimeFactory) *Runtime {
	t.Helper()
	workspace := t.TempDir()
	runtime := newRuntime(Config{AuthToken: "test-auth-token", Workspace: workspace, JSONContract: "one object"}, runtimeDependencies{
...
func TestStartRuntimeRequiresAndClonesEnvelopeSchema(t *testing.T) {
	for _, schema := range []json.RawMessage{nil, json.RawMessage(`[]`), json.RawMessage(`{"type":"object"} trailing`)} {
		runtime, err := StartRuntime(Config{AuthToken: "test-auth-token", EnvelopeSchema: schema})
		if runtime != nil || !errors.Is(err, agentruntime.ErrRuntimeConfiguration) || !strings.Contains(err.Error(), "nessy configure runtime") {
...
func TestRuntimeProductionFactoryUsesOneProcessAndSessionPerThread(t *testing.T) {
	...
	t.Setenv("STEPAN_NESSY_ACP_CASE", "structured-turn")
	...
	firstBackend := first.(*threadHandle).state.backend.(*nessyThread)
	secondBackend := second.(*threadHandle).state.backend.(*nessyThread)
...
func TestRuntimeProductionStartupReportsSafeExitDiagnostic(t *testing.T) {
	...
	t.Setenv("GO_WANT_NESSYAPP_FAKE", "startup-diagnostic")
	t.Setenv("STEPAN_NESSYAPP_DIAGNOSTIC_SECRET", secret)
	runtime, err := StartRuntime(Config{
		AuthToken:      token,
		Workspace:      workspace,
		JSONContract:   testJSONContract,
	...
	for _, want := range []string{`executable "nessy"`, "exit code 23", "during initialize lifecycle", "authentication"} {
...
func TestRuntimeProductionStartupReportsSafeRPCDiagnostic(t *testing.T) {
	workspace := makeGitRoot(t)
	t.Setenv("GO_WANT_NESSYAPP_FAKE", "acp")
	t.Setenv("STEPAN_NESSY_ACP_CASE", "session-auth-error")
	t.Setenv("STEPAN_NESSY_ACP_OBSERVATION", filepath.Join(t.TempDir(), "observation.json"))
	runtime, err := StartRuntime(Config{
		AuthToken:      testAuthToken(t),
		Workspace:      workspace,
...
func TestRuntimeProductionStartupCancellationClosesHungACPProcess(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			t.Setenv("GO_WANT_NESSYAPP_FAKE", "acp")
			t.Setenv("STEPAN_NESSY_ACP_CASE", test.scenario)
			t.Setenv("STEPAN_NESSY_ACP_OBSERVATION", observation)
			runtime, err := StartRuntime(Config{
				AuthToken: testAuthToken(t), Workspace: workspace, JSONContract: testJSONContract,
				EnvelopeSchema: json.RawMessage(`{"type":"object"}`),
			})
			if err != nil {
				t.Fatal(err)
...
func TestNessySafeErrorClassificationMatrix(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !strings.Contains(err.Error(), "nessy") || strings.Contains(err.Error(), secret) {
				t.Fatalf("unsafe Nessy diagnostic = %q", err)
			}
		})
	}
}

func TestNessySafeErrorRetainsOnlyAllowlistedCapabilityContext(t *testing.T) {
	const secret = "credential-body-do-not-echo"
...
func TestNessySafeErrorRetainsOnlyAllowlistedProtocolContext(t *testing.T) {
	const secret = "credential-body-do-not-echo"
...
func TestNessySafeErrorAddsUnclassifiedProtocolFallback(t *testing.T) {
	const secret = "credential-body-do-not-echo"
