...
func runNessyACPFake() int {
	...
	scenario := os.Getenv("STEPAN_NESSY_ACP_CASE")
	var sharedScript conformance.Script
	if scenario == "conformance" {
		data, readErr := os.ReadFile(os.Getenv("STEPAN_NESSY_CONFORMANCE_SCRIPT"))
...
func writeACPObservation(observation acpObservation) error {
	...
	return os.WriteFile(os.Getenv("STEPAN_NESSY_ACP_OBSERVATION"), data, 0o600)
}
...
func TestOpenConnectionPreflightsWithoutAdditionalDirectories(t *testing.T) {
	...
	t.Setenv("STEPAN_NESSY_ACP_CASE", "success")
	t.Setenv("STEPAN_NESSY_ACP_OBSERVATION", observationPath)
	process := NewProcess(Config{AuthToken: testAuthToken(t), Workspace: workspace, JSONContract: testJSONContract}, t.TempDir())
...
func TestOpenConnectionClosesProcessOnMissingCapability(t *testing.T) {
	for _, scenario := range []string{"missing-capability", "empty-capabilities"} {
		t.Run(scenario, func(t *testing.T) {
			t.Setenv("GO_WANT_NESSYAPP_FAKE", "acp")
			t.Setenv("STEPAN_NESSY_ACP_CASE", scenario)
...
func TestOpenConnectionClosesProcessOnSessionContractFailure(t *testing.T) {
	t.Setenv("GO_WANT_NESSYAPP_FAKE", "acp")
	t.Setenv("STEPAN_NESSY_ACP_CASE", "session-error")
	t.Setenv("STEPAN_NESSY_ACP_OBSERVATION", filepath.Join(t.TempDir(), "observation.json"))
	process := NewProcess(Config{AuthToken: testAuthToken(t), Workspace: makeGitRoot(t), JSONContract: testJSONContract}, t.TempDir())
	if err := process.Start(); err != nil {
		t.Fatal(err)
