...
func runResumeRequiredChecks(ctx context.Context, input ResumeInput, selection setting.CheckSelection) (ResumeRequiredChecksResult, error) {
	...
	if err := input.Run.AddRunOperation(implstate.Operation{
		ID: operationID, Kind: implstate.OperationCheck, Basis: basis,
		Description: "resume required checks", Counter: implstate.CycleCounterNone,
	...
	publisher, err := NewCheckResultPublisherWithControl(input.Journal, input.Workspace, input.Repository, implstate.EvidenceID(resultID))
	if err != nil {
	...
	if runErr != nil || !set.Succeeded() {
		...
		if errors.Is(checkContext.Err(), context.Canceled) || errors.Is(checkContext.Err(), context.DeadlineExceeded) {
			status = implstate.ResultInterrupted
		}
	}
	...
	if status == implstate.ResultSucceeded && input.Run.NeedsInitialBaselineRefresh() {
...
func nextResumeCheckIDs(run *implstate.Run) (implstate.OperationID, implstate.ResultID) {
	operations := make(map[implstate.OperationID]struct{})
	results := make(map[implstate.ResultID]struct{})
	if run != nil {
	...
	for number := 1; ; number++ {
		operation := implstate.OperationID(fmt.Sprintf("resume-required-check-%d", number))
		result := implstate.ResultID(fmt.Sprintf("resume-required-check-%d-result", number))
