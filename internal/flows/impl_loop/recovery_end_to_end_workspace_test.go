...
"test_all":{"kind":"tests","command":{"program":"test_all","args":[]}}}`, `["lint","test_all"]`), nil
	...
	t.Run("stale acceptance and a committed Git boundary recover without duplicate commits", func(t *testing.T) {
		...
		writeWorkspaceFile(t, filepath.Join(repository, "implementation.txt"), "accepted work survives recovery\n")
		...
		committed, err := disk.Commit(context.Background(), repository, message)
		if err != nil {
			t.Fatal(err)
		}
		created := committed.CommitID
		...
		result, err := ReconcilePendingCommit(context.Background(), ReconcilePendingCommitInput{
			Run: recovered, StateStore: recoveredState, Repository: repository, AssignmentID: "assignment", Observer: disk,
		})
		if err != nil || !result.Adopted || result.Commit.CommitID != created {
			t.Fatalf("recover committed boundary result=%#v err=%v", result, err)
		}

		current, err := disk.Observe(context.Background(), repository)
		if err != nil || current.CommitID != created {
			t.Fatalf("recovery duplicated commit: %#v, %v", current, err)
		}
