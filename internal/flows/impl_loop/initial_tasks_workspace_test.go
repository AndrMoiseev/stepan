...
func TestBeginNewChangeRejectsPreviouslyClosedRunAndLoadsOnlyFreshChange(t *testing.T) {
	...
	start, err := BeginNewChangeWithWorkspace(context.Background(), testfs.Directory(repository), store, repository, "fresh")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := BeginNewChangeWithWorkspace(context.Background(), testfs.Directory(repository), store, repository, "change"); !errors.Is(err, ErrChangeAlreadyStarted) {
...
func TestContinueOwnRunUsesOnlyCurrentWorkingCopyAndNeverClosedRun(t *testing.T) {
	store := mustControllerStore(t, t.TempDir())
	ownerRepository := newFilesystemWorkspace(t)
	otherRepository := newFilesystemWorkspace(t)
	...
	resumed, err := ContinueOwnRunWithWorkspace(context.Background(), testfs.Directory(ownerRepository), store, ownerRepository)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := ContinueOwnRunWithWorkspace(context.Background(), testfs.Directory(otherRepository), store, otherRepository); !errors.Is(err, ErrNoResumableRun) {
...
func TestForeignUnavailableRunsDoNotBlockCurrentWorkCopy(t *testing.T) {
	store := mustControllerStore(t, t.TempDir())
	current := newFilesystemWorkspace(t)
	foreign := newFilesystemWorkspace(t)
	...
	start, err := BeginNewChangeWithWorkspace(context.Background(), testfs.Directory(current), store, current, "fresh")
	if err != nil {
		t.Fatal(err)
	}
	if err := start.Lease.Close(); err != nil {
