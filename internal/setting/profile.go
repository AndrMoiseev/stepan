...
func (configuration Configuration) ResolveRoleProfile(role string) (RuntimeProfile, error) {
	...
	if _, ok := configuration.Profiles[name]; !ok {
		return RuntimeProfile{}, fmt.Errorf("implementation configuration: role %q references missing profile %q", role, name)
	}
	return configuration.ResolveProfile(name)
}

// ResolveProfile makes the shared profile registry available to any flow
// without exposing the JSON representation or requiring impl_loop roles.
func (configuration Configuration) ResolveProfile(name string) (RuntimeProfile, error) {
	raw, ok := configuration.Profiles[name]
	if !ok {
		return RuntimeProfile{}, fmt.Errorf("settings: missing profile %q", name)
	}
